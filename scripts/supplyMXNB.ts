...
async function main() {
  ...
  try {
    ...
    const mockMXNB = new ethers.Contract(CONTRACT_ADDRESSES.mockMXNB, MOCK_WETH_ABI, deployer);
    ...
    const mockMXNBERC20 = new ethers.Contract(CONTRACT_ADDRESSES.mockMXNB, ERC20_ABI, deployer);

    console.log(`Approving ${ethers.formatUnits(DEPOSIT_AMOUNT, 6)} MXNB to Vault (${VAULT_ADDRESS})...`);
    const approveTx = await mockMXNBERC20.approve(VAULT_ADDRESS, DEPOSIT_AMOUNT);
