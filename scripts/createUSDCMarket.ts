...
const CONTRACT_ADDRESSES = {
  ...
  EthUsdcOracle: "0x3A19A14c709cf798E330277ed942475EBadAC74E", // From deploy.ts output
};
...
const VAULT_CONFIG = {
  ...
  supplyCapAmount: ethers.parseUnits("100000000", 6), // 100000000 USDC cap
};
...
async function main() {
  ...
  if (!CONTRACT_ADDRESSES.USDC.startsWith("0x") || CONTRACT_ADDRESSES.USDC === "0x") {
    throw new Error("ERROR: USDC address not set in CONTRACT_ADDRESSES. Update it from deploy.ts output.");
  }
  if (!CONTRACT_ADDRESSES.WETH.startsWith("0x") || CONTRACT_ADDRESSES.WETH === "0x") {
    throw new Error("ERROR: WETH address not set in CONTRACT_ADDRESSES. Update it from deploy.ts output.");
  }
  if (!CONTRACT_ADDRESSES.EthUsdcOracle.startsWith("0x") || CONTRACT_ADDRESSES.EthUsdcOracle === "0x") {
    throw new Error("ERROR: EthUsdcOracle address not set in CONTRACT_ADDRESSES. Update it from deploy.ts output.");
  }
  ...
  try {
    ...
    // ============================================================================
    console.log("[3/3] Creating Morpho Vault for USDC management...");
    ...
    try {
      const createVaultTx = await vaultFactory.createMetaMorpho(
        ...
        CONTRACT_ADDRESSES.USDC,  // asset (USDC)
    ...
    if (vaultAddress) {
      ...
      console.log(`Setting allocator with market: ${marketId}`);
      try {
    ...
    const marketDetailsFile = path.join(__dirname, "../market-details-usdc.json");
    const marketDetails = {
      marketId,
